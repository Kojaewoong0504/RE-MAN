...
- 첫 화면은 사진/텍스트 기준, 추천 조합, 오늘 행동을 먼저 보여준다. 진단은 추천을 이해하는 짧은 보조 문장으로 둔다.
...
- `조합 느낌 보기`는 생성 기능이 아니라 레퍼런스/준비 안내 패널이다. 이 패널은 `/api/try-on`을 호출하거나 크레딧을 차감하면 실패다.
- 옷장 근거는 추천 조합 바로 아래에서 `추천에 직접 사용`, `가장 가까운 옷`, `있으면 추가`처럼 역할을 짧게 보여준다.
- 사이즈 후보와 내부 기준 후보는 보조 정보로 접어 둔다. 사용자가 `사이즈 후보 보기`를 열었을 때만 표시한다.
- 사이즈 정보가 없으면 접힌 보조 패널 안에서 `사이즈 추가하기`를 `/settings#size-profile`로 연결한다.
- 결과 화면에서 서버 호출을 만드는 보조 기능 버튼을 여러 개 두지 않는다. 대기 중 사용자가 다른 생성성 기능을 누를 수 있으면 실패다.
- 추천 조합 카드 이후에는 옷장 근거, 새 체크/옷장/기록, 개선 포인트, 사이즈 후보, 저장/피드백만 남긴다.
...
- 생성 없는 `조합 느낌 보기` 패널은 허용한다. 단, 버튼/문구가 실제 실착 생성처럼 보이거나 provider 호출을 만들면 실패다.
