...
- E2E는 이 로딩 문구를 필수 도착 상태로 검증하지 않는다. 응답이 빠르면 `/analyzing` 문구가 보이기 전에 `/result`로 이동할 수 있으므로, 분석 요청/응답과 최종 결과를 안정 조건으로 검증한다.
...
### 옷장 대량 리뷰 (`/closet/review`)
- 리뷰 카드마다 상의/하의/신발/겉옷 빠른 분류 버튼을 제공한다.
- 빠른 분류만 눌러도 해당 초안은 저장 가능한 `confirmed` 상태가 되어야 한다.
- 이름이 비어 있으면 `{종류} 사진` 기본 이름을 붙인다. 사용자가 모든 초안의 이름을 직접 입력해야 저장되면 실패다.
- 저장 성공 후 `/closet?from=review&saved=N`으로 이동해 저장 개수와 스타일 체크 준비 상태를 보여준다.
- 준비 완료 상태에서 `이 옷장으로 스타일 체크`를 누르면 기존 설문/옷장 컨텍스트를 유지하고 `/programs/style/onboarding/upload`로 이어져야 한다.
