...
- 내 정보 화면은 상태 확인 허브다. 주요 행동은 `새 체크`, `옷장`, `기록` 3개로 제한하고, 설정/로그아웃은 계정 관리 보조 영역으로 분리한다.
...
- 결과 화면은 추천 조합, 근거 옷, 오늘 행동, 반응 저장을 기본 화면에 직접 보여준다.
- 결과 화면에서 진단 상세, 개선점 목록, 사이즈 후보, 계정 저장, 실착/레퍼런스 준비 패널을 별도 CTA로 늘리지 않는다.
