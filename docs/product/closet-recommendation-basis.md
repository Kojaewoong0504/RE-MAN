...
`추천에 사용`은 현재 요청의 `closet_items`에서 같은 카테고리 id가 검증된 경우에만 함께 `옷장 ID 검증`을 표시한다.
텍스트 매칭이나 fallback 근거를 같은 수준으로 보이게 만들면 실패다.

## Hybrid Recommendation Rule

- 결과 화면은 `내 옷장 기준`과 `시스템 추천`을 별도 블록으로 분리한다.
- `recommendation_mix.primary_source = closet`이면 `내 옷장 기준` 블록이 먼저 와야 한다.
- `recommendation_mix.primary_source = system`이면 `시스템 추천` 블록이 먼저 와야 한다.
- `system_recommendations`는 참고용 `reference`만 허용한다. 사용자에게는 `시스템 추천 참고` 같은 출처 라벨을 짧게 보여준다.
- 시스템 추천 블록에 구매 CTA, 가격, 재고, 외부 상품 링크를 붙이면 실패다.
- 시스템 추천이 비어 있어도 `recommendation_mix.summary`는 남아야 하며, 옷장 중심 추천을 보조 설명하는 데만 사용한다.
...
- hybrid recommendation 순서와 출처 라벨은 `npm run test:e2e -- --grep "hybrid recommendation"`으로 확인한다.
...
- `system_recommendations`를 사용하면서 출처 라벨 없이 옷장 추천과 같은 수준의 확정 추천처럼 보이게 만들면 실패다.
