...
- 모바일/IAB 로그인 경로는 popup 우선 방식으로 보고하지 않는다. 모바일 또는 인앱 브라우저에서는 redirect 우선 로그인을 사용하고, popup fallback만으로 해결됐다고 말하지 않는다.
...
- `try-on`은 사람 사진 1장 + 추천 조합 이미지 1장만 허용하며, 사용자가 명시적으로 요청할 때만 호출
- 추천 조합 이미지는 상의/하의/신발을 한 장으로 합성한 outfit board를 사용할 수 있다. 단, 여러 API 입력 이미지를 직접 넘기면 실패다.
