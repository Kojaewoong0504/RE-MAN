...
function getJsonShape(agent: AgentKind) {
  ...
  "recommendation_mix": {
    ...
    "summary": "옷장 기준 추천을 유지하고 부족한 부분은 시스템 추천으로 보강합니다."
  },
...
${
  agent === "fit"
    ...
    : agent === "color"
      ...
      : "onboarding-agent는 recommended_outfit, recommendation_mix, system_recommendations를 모두 반드시 포함하세요. 추천 조합은 현재 옷장 컨텍스트를 먼저 사용하고 구매를 유도하지 마세요. system_recommendations는 reference 전용이고 product는 null로 두세요. preference_profile의 좋아한 방향은 유지하고 피할 방향은 반복하지 마세요. 등록된 옷장 아이템을 사용했다면 recommended_outfit.source_item_ids에 해당 id를 카테고리별로 넣으세요.",
